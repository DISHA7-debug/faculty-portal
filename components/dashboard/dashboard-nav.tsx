...
function isActive(pathname: string, href: string): boolean {
  // Overview must match exactly, or it stays highlighted on every child route.
  if (href === '/dashboard') return pathname === '/dashboard';
  return pathname === href || pathname.startsWith(`${href}/`);
}

export function DashboardNav({ counts }: { counts?: Record<string, number> }) {
  ...
  return (
    <nav aria-label="Profile sections">
      {/*
        Horizontal and scrollable below `lg`, vertical in the sidebar above it. The mobile
        form is a scroller rather than a menu so the current section stays visible without
        a tap.
      */}
