...
import { courseSchema, reorderSchema } from '@/lib/validation/sections';
 ...
 *
 * The same course is often taught across many years; each entry is one offering.
 */
...
async function listFor(profileId: string): Promise<CourseRow[]> {
...
export async function reorderCourseAction(
  ...
  await db.$transaction(
    ids.map((id, index) =>
      db.course.update({ where: { id }, data: { sortOrder: index } }),
    ),
  );
...
export async function listCourse(): Promise<CourseRow[]> {
  const session = await requireSession();
